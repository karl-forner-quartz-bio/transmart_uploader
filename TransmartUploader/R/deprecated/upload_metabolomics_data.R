...
#' upload metabolopmics data (high dimensional data) using tMDataLoader
#'
...
#' @author Sepideh
#' @export
upload_metabolomics_data <- function(
  platform,
  df,
  study_id,
  map_df,
  tissue_type = " ",
  transmart_path = paste0("Tests", "_", study_id),		## THIS is a default parameter. If we forget, we wont overwrite.
  ...
) {
 
  #add "...ToUpload" to the path
  transmart_path = paste0(transmart_path, "/MetabolomicsDataToUpload")

  res <- run_tm_etl_for_metabolomics_data(platform = platform, data_dfs= df, map_df = map_df, etl_path = transmart_path)
  ## res <- run_tm_etl_for_expression_data(platform = platform, data_dfs= df, map_df = map_df, etl_path = transmart_path)
